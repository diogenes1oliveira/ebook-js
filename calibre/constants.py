...
def get_osx_version():
    ...
    if _osx_ver is None:
        ...
        try:
            ...
            if len(ver) == 2:
                ver.append(0)
            _osx_ver = OSX(*list(map(int, ver)))  # no2to3
        except Exception:
            _osx_ver = OSX(0, 0, 0)
    return _osx_ver
...
ROOT_DIRECTORY = str(Path(__file__).absolute().parent)
plugins_loc = os.environ.get('CALIBRE_SETUP_EXTENSIONS_PATH', os.path.join(ROOT_DIRECTORY, 'calibre', 'plugins'))
